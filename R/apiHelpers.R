...
writeSiteLog <- function(Dataset, siteID, PropertyType, df, usr, logDir){

  if(!dir.exists(logDir)){dir.create(logDir)}
  logFile <- paste0(logDir,'/soilDataFederatorSiteDataUsage.csv')
  if(!file.exists(logFile)){
    cat('DateTime,User,Dataset,SiteID,PropertyType,Count\n', sep = '', file = logFile, append = F)
  }

  if(nrow(df) > 0){
    odf <- data.frame(DateTime=now(),User=usr,Dataset=Dataset,SiteID=siteID,PropertyType=PropertyType, Count=nrow(df), stringsAsFactors = F)
    write.table(odf, logFile,append = TRUE,sep = ",",col.names = FALSE, row.names = FALSE,  quote = FALSE)
  }
}
